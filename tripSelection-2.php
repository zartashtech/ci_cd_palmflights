<?php
$london_all="London, All Airports [LON], United Kingdom";
$london_city ="London, London City Arpt [LCY], United Kingdom";
$london_gatwick="London, London Gatwick Arpt [LGW], United Kingdom";
$london_hathrow="London, London Heathrow Arpt [LHR], United Kingdom";
$manchester="Manchester, Manchester Intl [MAN], United Kingdom";
$birmingham="Birmingham, Birmingham Intl Arpt [BHX], United Kingdom";
...
include('connection-db.php');
	...
	if ($_SESSION['trip_type']=='oneway') //select data in one way
	{
		if ($_SESSION['airline_name']=="ALL") //(one way)if preffered airline selected
			{
				if (isset($_SESSION['flight_route'])) //(one way)preffered and direct airline selected
				{
					...
					(origin='$london_city' 
        OR origin='$london_gatwick' 
        OR origin='$london_hathrow'
        ) and destination='$_SESSION[destination]' and class_name='$_SESSION[class_name]' ORDER BY total_realtime_fare ASC";
				}
					
				else //(one way)preffered and indirect airline selected
				{
					$sql = "SELECT * FROM flight_detail where (origin='$london_city' 
        OR origin='$london_gatwick' 
        OR origin='$london_hathrow'
        ) and destination='$_SESSION[destination]' 
        ...
        OR origin='$london_gatwick' 
        OR origin='$london_hathrow'
        ) and trip_type='$_SESSION[trip_type]' and destination='$_SESSION[destination]' and class_name='$_SESSION[class_name]' ORDER BY total_realtime_fare ASC";
				}
	...
	else //(two way)
	{
        ...
        WHERE (origin='$london_city' 
        OR origin='$london_gatwick' 
        OR origin='$london_hathrow'
        ) 
        AND trip_type='$_SESSION[trip_type]' 
        AND destination='$_SESSION[destination]' 
        AND class_name='$_SESSION[class_name]' 
        ORDER BY total_realtime_fare ASC";
				}				
			}
