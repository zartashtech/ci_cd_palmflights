...
<form id="form-booking-via-direct-landing-page" action="<?php echo strip_tags(htmlspecialchars("emails-files/vr3-recaptcha/captcha-booking-via-direct-landing-page.php?origin=$_GET[origin]&destination=$_GET[destination]&origin_city=$_GET[origin_city]&destination_city=$_GET[destination_city]&trip_type=$_GET[trip_type]&duration=$_GET[duration]&stopCount=$_GET[stopCount]&totalFare=$_GET[totalFare]&class_name=$_GET[class_name]&adults=$_GET[adults]&child=$_GET[child]&infants=$_GET[infants]&flexible_date=$_GET[flexible_date]&airline_name=$_GET[airline_name]")); ?>" method="post">
<input type="hidden" name="form_name" value="book-via-direct-lp"> 
    <div class="row">
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <div>
                <input type="hidden" name="g-token" id="g-token" value="save"  />
            </div>
        </div>
    </div>
    ...
    <input class="btn btn-primary" name="btnSubmit" type="submit" value="Complete Booking" />
</form>

                                        <script>
    $('#form-booking-via-direct-landing-page').submit(function(event) {
        event.preventDefault();
        var email = $('#email').val();
 
        grecaptcha.ready(function() {
            grecaptcha.execute('6Le70_0lAAAAAEEnPWZ5M5HQ5L7tjetfP49Y2DbW', {action: 'subscribe_newsletter'}).then(function(token) {
                $('#form-booking-via-direct-landing-page').prepend('<input type="hidden" name="token" value="' + token + '">');
                $('#form-booking-via-direct-landing-page').prepend('<input type="hidden" name="action" value="subscribe_newsletter">');
                $('#form-booking-via-direct-landing-page').unbind('submit').submit();
            });;
        });
  });
  </script>
